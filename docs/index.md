...
This project is everything except that operation — the library, the orchestration, the container and the CI — so a cache repository holds only what makes it different from its siblings.
...
- **Setting up a new cache?** Generate it from [`cache-template`](https://github.com/dandi-cache/cache-template) and follow its `setup-cache` skill.
